...
|---|---|---|---|---|
| A task is claimed exactly once | `loop.claim_task` under `_state_lock` **and `loop.adopt_task` for the resume path** | `test_lock`, `test_e2e`, `test_audit` | Two tasks on one course serialise; both complete; no lock file remains. Six tasks under two concurrent loops produce six `task_end` events, not twelve | Until U15 this row was **false and the test could not tell**: it counted `task_start` lines, which the resume path never emits, so a task taken from a live sibling and run twice passed the check. Ownership is now recorded on the task and re-checked under the mutex. What remains unestablished is behaviour across two *hosts* sharing one filesystem, where liveness cannot be asked and only the lease applies |
