...
Generated 2026-09-03T00:57:21 from an actual suite run: **148/150 tests passed**, **793 observations** recorded.
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13325 token window in 1.7s, cut marked
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 12808m > 1m
...
- `test_runbook.py` **[goal g-20260902-212850]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260902-212850]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260902-212930]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260902-212930]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
- `test_package.py` **[runnable]** the archive carries 117 modules, 150 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[evidence]** all 150 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1788398679-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 78,821 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 19 capabilities; the README badges carry 150 tests and 32 mutations
...
- `test_invariants.py` **[execution]** 113 modules scanned; 0 raw subprocess sites outside the authority (18 declared platform-internal, each with a stated reason)
...
- `test_invariants.py` **[metering]** 113 modules scanned by AST; every function that reaches a model provider meters it (2 declared free, each with a stated reason and each still present in the source)
...
- `test_invariants.py` **[sandboxes]** 185 sandbox names across 150 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
...
- `test_invariants.py` **[prose]** ARCHITECTURE/README/REFERENCE counts verified against the tree itself: 113 modules, 150 acceptance tests
...
- `test_controlplane.py` **[cost]** a 1910-path control plane (1500 approvals, 200 goal ledgers) seals and verifies in 137 ms per command — 27 s before the caches — and a change to a cached path is still caught
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 45.2s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.0s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname '51aee0189abf' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94595 -> 198707 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.15s, 0.17s, 0.17s, 0.16s, 0.16s, 0.16s — median 0.16s, and the last batch is not an outlier: the loop does not get slower as its own history grows
