...
Generated 2026-09-03T12:06:59 from an actual suite run: **151/154 tests passed**, **814 observations** recorded.
...
| 4. Memory institution | **proven** | 23/23 | 89 |
...
| 9. Computers, capability and organization | **proven except skipped** | 6/8 | 37 |
| 10. Training lab | **proven** | 1/1 | 6 |
| 12. The paths that touch something real | **proven** | 6/6 | 34 |
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 13679m > 1m
...
- `test_runbook.py` **[goal g-20260903-115944]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260903-115944]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260903-120020]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260903-120020]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
**Verdict: proven** — 23 of 23 declared tests ran and passed, producing 89 observations.

<details><summary>What the tests observed (89)</summary>
...
- `test_twin.py` **[learning]** a synthetic owner (alice always yes; deny when risk > 0.5 and margin < 0.3) gave held-out choice fidelity 0.96 on 23 rows; attention ranked cp:alice, sit:margin, sit:risk; 29 habits proven including the alice rule; the fit is byte-stable; a random owner scored 0.46 and the verdict was 'low', not 'high'
- `test_twin.py` **[calibration]** Brier 0.0938, ECE 0.0725 and a high-confidence error rate are computed on held-out rows only (a contaminated split is refused); an unseen situation carried novelty 1.0 (tier low) and ask mass 0.6769 against 0.0 / 0.0004 for a known one
...
- `test_frontend.py` **[serve]** page served from ui.html (338805 bytes)
...
- `test_package.py` **[runnable]** the archive carries 121 modules, 154 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[evidence]** all 154 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1788451014-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 78,835 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 20 capabilities; the README badges carry 154 tests and 36 mutations
...
- `test_invariants.py` **[execution]** 117 modules scanned; 0 raw subprocess sites outside the authority (18 declared platform-internal, each with a stated reason)
...
- `test_invariants.py` **[metering]** 117 modules scanned by AST; every function that reaches a model provider meters it (2 declared free, each with a stated reason and each still present in the source)
...
- `test_invariants.py` **[control-plane]** the seal is derived from fileauth's zone model: all 35 control shapes sealed, the workspace untouched, every path with a declared treatment
...
- `test_invariants.py` **[sandboxes]** 189 sandbox names across 154 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
...
- `test_invariants.py` **[prose]** ARCHITECTURE/README/REFERENCE counts verified against the tree itself: 117 modules, 154 acceptance tests
...
- `test_promotion_leakage.py` **[static]** all 37 trust-defining paths classify CONTROL or RUNTIME — none is workspace
- `test_promotion_leakage.py` **[dynamic]** the file authority refused an agent write to every one of the 37 paths and still admits the harness
...
- `test_controlplane.py` **[matrix]** 116 shell commands from a role holding run_command — truncate, append, delete, create and a redirect — against all 35 control paths fileauth declares: not one durable change, every attempt reported exit=3, 116 tamper events on the record
...
- `test_controlplane.py` **[docker]** 34 read-only bind(s) layered over /work cover every one of the 35 control paths; on that backend the boundary is the kernel's, not a check's
...
- `test_controlplane.py` **[cost]** a 1910-path control plane (1500 approvals, 200 goal ledgers) seals and verifies in 140 ms per command — 27 s before the caches — and a change to a cached path is still caught
...
**Verdict: proven except skipped** — 6 of 8 declared tests ran and passed, producing 37 observations.
...
<details><summary>What the tests observed (37)</summary>
...
**Verdict: proven** — 6 of 6 declared tests ran and passed, producing 34 observations.
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94624 -> 198649 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.13s, 0.16s, 0.16s, 0.16s, 0.16s, 0.16s — median 0.16s, and the last batch is not an outlier: the loop does not get slower as its own history grows
...
- `test_endurance.py` **[ledgers]** the whole expert directory is 2.0 MB after 120+ tasks (16.8 KB per task): the model gateway 48 KB, routing outcomes 27 KB, compiled context windows 1116 KB
...
- `test_endurance.py` **[soak]** 21s of continuous operation. This is minutes, not weeks: it rules out the growth that is O(total work), and it cannot rule out a leak that needs days to show.
