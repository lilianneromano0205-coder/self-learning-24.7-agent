...
Generated 2026-09-04T22:51:42 from an actual suite run: **151/155 tests passed**, **831 observations** recorded.
...
| 12. The paths that touch something real | **proven except skipped** | 5/6 | 33 |
...
- `test_team.py` **[team t-test]** done -> C:/Users/redab/AppData/Local/Temp/agent-measurement-suite-final\team\teamwork\t-test\result.md
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 15761m > 1m
...
- `test_contract.py` **[goal g-accept]** turn this success into deterministic capability: python runbook.py draft C:/Users/redab/AppData/Local/Temp/agent-measurement-suite-final\contract\experts\honest-builder g-accept (then fill the TODO steps and let three verified runs promote it)
...
- `test_runbook.py` **[goal g-20260904-224123]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260904-224123]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260904-224157]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260904-224157]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
- `test_package.py` **[runnable]** the archive carries 122 modules, 155 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1788575897-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 80,072 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 20 capabilities; the README badges carry 155 tests and 49 mutations
...
**Verdict: proven except skipped** — 5 of 6 declared tests ran and passed, producing 33 observations.
**NOT RUN HERE — test_docker_live.py:** docker is not available on this machine, which is a legitimate installation. `python sandbox.py` reports the same thing, and the host backend is used instead.
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94595 -> 198621 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.14s, 0.16s, 0.16s, 0.17s, 0.17s, 0.17s — median 0.16s, and the last batch is not an outlier: the loop does not get slower as its own history grows
