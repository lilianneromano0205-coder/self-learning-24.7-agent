...
Generated 2026-08-27T14:53:21 from an actual suite run: **114/116 tests passed**, **577 observations** recorded.
...
|---|---|---|---|
| 1. Harness & loop | **proven except skipped** | 23/24 | 87 |
...
| 6. Control plane & interop | **proven** | 19/19 | 116 |
| 7. The five authorities | **proven** | 1/1 | 19 |
| 8. Proof, missions and long-horizon work | **proven** | 4/4 | 27 |
| 9. Computers, capability and organization | **proven except skipped** | 3/4 | 21 |
...
**Verdict: proven except skipped** — 23 of 24 declared tests ran and passed, producing 87 observations.
...
<details><summary>What the tests observed (87)</summary>
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13234 token window in 1.3s, cut marked
...
- `test_retention.py` **[flat]** the hot state is capped at 50 finished task(s) after 400 more were run (limit 65), so persist is bounded work forever — measured 10 ms -> 12 ms here, but the COUNT is the guarantee and the clock is only a smoke check
...
**Verdict: proven** — 19 of 19 declared tests ran and passed, producing 116 observations.

<details><summary>What the tests observed (116)</summary>
...
- `test_frontend.py` **[serve]** page served from ui.html (311116 bytes)
...
- `test_package.py` **[runnable]** the archive carries 89 modules, 116 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[evidence]** all 116 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1787856418-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 75,336 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_invariants.py` **[execution]** 85 modules scanned; 0 raw subprocess sites outside the authority (16 declared platform-internal, each with a stated reason)
...
- `test_invariants.py` **[sandboxes]** 162 sandbox names across 116 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
...
**Verdict: proven except skipped** — 3 of 4 declared tests ran and passed, producing 21 observations.
...
- `test_rbac.py` **[coverage]** 21 POST routes; 19 named in the table and 2 falling through to 'create_agent', which needs builder or above — a route added tomorrow is refused for a viewer, not waved through
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 32422 -> 68004 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.12s, 0.12s, 0.13s, 0.13s, 0.13s, 0.13s — median 0.13s, and the last batch is not an outlier: the loop does not get slower as its own history grows
...
- `test_endurance.py` **[ledgers]** the whole expert directory is 1.3 MB after 120+ tasks (11.1 KB per task): the model gateway 48 KB, routing outcomes 24 KB, compiled context windows 972 KB
