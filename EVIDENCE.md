...
Generated 2026-09-02T15:09:45 from an actual suite run: **145/147 tests passed**, **772 observations** recorded.
...
|---|---|---|---|
| 1. Harness & loop | **proven except skipped** | 27/28 | 113 |
| 2. Fleet & creation lanes | **proven** | 6/6 | 30 |
| 3. Work systems | **proven** | 23/23 | 152 |
| 4. Memory institution | **proven** | 22/22 | 76 |
| 5. Improvement & governance | **proven** | 19/19 | 92 |
...
**Verdict: proven except skipped** — 27 of 28 declared tests ran and passed, producing 113 observations.
...
- `test_harness.py` **[manifest]** 14 tools with role allowlists, 9+ gates, policies, 14 memory tiers, budgets, events, file hashes - all read from what runs
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13318 token window in 1.4s, cut marked
...
**Verdict: proven** — 23 of 23 declared tests ran and passed, producing 152 observations.

<details><summary>What the tests observed (152)</summary>
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 12422m > 1m
...
- `test_runbook.py` **[goal g-20260902-150220]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260902-150220]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260902-150255]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260902-150255]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
**Verdict: proven** — 19 of 19 declared tests ran and passed, producing 92 observations.

<details><summary>What the tests observed (92)</summary>
...
- `test_package.py` **[runnable]** the archive carries 116 modules, 147 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[evidence]** all 147 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1788375500-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 78,805 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_invariants.py` **[execution]** 112 modules scanned; 0 raw subprocess sites outside the authority (18 declared platform-internal, each with a stated reason)
...
- `test_invariants.py` **[metering]** 112 modules scanned by AST; every function that reaches a model provider meters it (2 declared free, each with a stated reason and each still present in the source)
...
- `test_invariants.py` **[control-plane]** the seal is derived from fileauth's zone model: all 31 control shapes sealed, the workspace untouched, every path with a declared treatment
...
- `test_invariants.py` **[sandboxes]** 184 sandbox names across 147 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
- `test_invariants.py` **[settings]** 66 [agent] key(s) read across the modules, every one of them declared in settings.toml — the file an operator reads is the file the code obeys
...
- `test_invariants.py` **[prose]** ARCHITECTURE/README/REFERENCE counts verified against the tree itself: 112 modules, 147 acceptance tests
...
- `test_promotion_leakage.py` **[static]** all 30 trust-defining paths classify CONTROL or RUNTIME — none is workspace
- `test_promotion_leakage.py` **[dynamic]** the file authority refused an agent write to every one of the 30 paths and still admits the harness
...
- `test_controlplane.py` **[matrix]** 103 shell commands from a role holding run_command — truncate, append, delete, create and a redirect — against all 31 control paths fileauth declares: not one durable change, every attempt reported exit=3, 103 tamper events on the record
...
- `test_controlplane.py` **[docker]** 30 read-only bind(s) layered over /work cover every one of the 31 control paths; on that backend the boundary is the kernel's, not a check's
...
- `test_controlplane.py` **[cost]** a 1910-path control plane (1500 approvals, 200 goal ledgers) seals and verifies in 143 ms per command — 27 s before the caches — and a change to a cached path is still caught
...
- `test_live_provider.py` **[wire]** one real HTTP call carried the model, the messages, the configured 4096-token ceiling, exactly the 14 tools this role is allowed, the bearer key and the configured extra header — and with the ceiling left at its default, max_tokens is omitted rather than sent as 0
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 45.6s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.1s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname 'db9318205b73' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94630 -> 198703 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.14s, 0.16s, 0.17s, 0.17s, 0.17s, 0.16s — median 0.17s, and the last batch is not an outlier: the loop does not get slower as its own history grows
...
- `test_endurance.py` **[ledgers]** the whole expert directory is 1.9 MB after 120+ tasks (16.6 KB per task): the model gateway 48 KB, routing outcomes 27 KB, compiled context windows 1090 KB
- `test_endurance.py` **[context]** across 42 compiled windows the median size went 1167 -> 1167 tokens: the window is bounded by its budget, not by how much the fleet remembers
