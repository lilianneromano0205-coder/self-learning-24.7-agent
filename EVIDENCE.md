...
Generated 2026-09-01T15:52:19 from an actual suite run: **136/138 tests passed**, **649 observations** recorded.
...
|---|---|---|---|
| 1. Harness & loop | **proven except skipped** | 26/27 | 94 |
...
**Verdict: proven except skipped** — 26 of 27 declared tests ran and passed, producing 94 observations.
...
<details><summary>What the tests observed (94)</summary>
...
- `test_package.py` **[runnable]** the archive carries 109 modules, 138 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[evidence]** all 138 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1788291753-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 78,759 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_invariants.py` **[sandboxes]** 176 sandbox names across 138 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
...
- `test_invariants.py` **[prose]** ARCHITECTURE/README/REFERENCE counts verified against the tree itself: 105 modules, 138 acceptance tests
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 45.0s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.2s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname '90fdfc6fbc56' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 93492 -> 196233 bytes (2.1x)
