...
Generated 2026-09-02T10:46:45 from an actual suite run: **143/145 tests passed**, **695 observations** recorded.
...
**Verdict: proven except skipped** — 27 of 28 declared tests ran and passed, producing 104 observations.
...
**Verdict: proven** — 18 of 18 declared tests ran and passed, producing 75 observations.

<details><summary>What the tests observed (75)</summary>
...
- `test_xlsx_operators.py` **[refusals]** formula, merged and error cells, a DOCTYPE, an escaping member, a missing sheet, an oversized grid, a non-workbook, a bad sheet name, a control character and a schema violation each refused by name before any side effect
...
- `test_package.py` **[runnable]** the archive carries 116 modules, 145 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[evidence]** all 145 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1788359792-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 78,800 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
**Verdict: proven** — 3 of 3 declared tests ran and passed, producing 36 observations.
...
- `test_invariants.py` **[sandboxes]** 183 sandbox names across 145 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
...
- `test_invariants.py` **[prose]** ARCHITECTURE/README/REFERENCE counts verified against the tree itself: 112 modules, 145 acceptance tests
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 46.0s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.0s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname '66b115cd60b4' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94634 -> 198702 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.12s, 0.14s, 0.15s, 0.15s, 0.15s, 0.16s — median 0.15s, and the last batch is not an outlier: the loop does not get slower as its own history grows
