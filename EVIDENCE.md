...
Generated 2026-09-02T14:30:09 from an actual suite run: **144/146 tests passed**, **765 observations** recorded.
...
| 5. Improvement & governance | **proven** | 18/18 | 85 |
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13318 token window in 1.5s, cut marked
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 12383m > 1m
...
- `test_runbook.py` **[goal g-20260902-142305]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260902-142305]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260902-142343]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260902-142343]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
**Verdict: proven** — 18 of 18 declared tests ran and passed, producing 85 observations.
...
- `test_package.py` **[runnable]** the archive carries 116 modules, 146 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[evidence]** all 146 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1788373154-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 78,806 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_invariants.py` **[sandboxes]** 183 sandbox names across 146 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
...
- `test_invariants.py` **[prose]** ARCHITECTURE/README/REFERENCE counts verified against the tree itself: 112 modules, 146 acceptance tests
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname 'eea0b5581b36' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94634 -> 198698 bytes (2.1x)
