...
Generated 2026-08-31T12:10:52 from an actual suite run: **134/136 tests passed**, **636 observations** recorded.
...
|---|---|---|---|
| 1. Harness & loop | **proven except skipped** | 25/26 | 90 |
...
| 7. The six authorities | **proven** | 3/3 | 31 |
...
**Verdict: proven except skipped** — 25 of 26 declared tests ran and passed, producing 90 observations.
...
- `test_frontend.py` **[serve]** page served from ui.html (324506 bytes)
...
- `test_package.py` **[runnable]** the archive carries 108 modules, 136 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[evidence]** all 136 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1788191933-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 77,687 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
**Verdict: proven** — 3 of 3 declared tests ran and passed, producing 31 observations.
...
- `test_invariants.py` **[sandboxes]** 175 sandbox names across 136 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname 'e4336968363b' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_first_day.py` **[first-task]** with the probe green, a gated task ran to completion over the same provider — the artefact exists, the gate passed, and the key appears nowhere in 3158 characters of log
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 93488 -> 196317 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.13s, 0.16s, 0.16s, 0.16s, 0.17s, 0.16s — median 0.16s, and the last batch is not an outlier: the loop does not get slower as its own history grows
