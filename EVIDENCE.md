...
Generated 2026-08-27T10:03:55 from an actual suite run: **114/116 tests passed**, **574 observations** recorded.
...
|---|---|---|---|
| 1. Harness & loop | **proven except skipped** | 23/24 | 86 |
| 2. Fleet & creation lanes | **proven** | 6/6 | 21 |
| 3. Work systems | **proven** | 22/22 | 109 |
...
**Verdict: proven except skipped** — 23 of 24 declared tests ran and passed, producing 86 observations.
...
<details><summary>What the tests observed (86)</summary>
...
**Verdict: proven** — 22 of 22 declared tests ran and passed, producing 109 observations.

<details><summary>What the tests observed (109)</summary>
...
- `test_freshness.py` **[freshness]** 5 atoms scanned: the expired one flagged with both dates, the superseded one flagged naming its successor, the retracted source flagged through the CONTROL-zoned ledger (short refs refused, worker locked out), nothing deleted, and the Crossref retraction verdict proven pure and offline
...
- `test_frontend.py` **[serve]** page served from ui.html (301169 bytes)
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1787839037-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 75,333 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_live_provider.py` **[retry-after]** a 429 asking for 45s slept 45.0s (the blind backoff would have been 2s and retried into a closed window), a 503 asking for 1s slept 1.0s instead of 2s or more, and both carry jitter so simultaneous experts do not return in lockstep
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 32420 -> 68015 bytes (2.1x)
...
- `test_ux.py` **[mobile]** the sidebar becomes a bottom bar with 40px targets, grid items may shrink, and every one of the 40 tables sits in a scroll container
