...
Generated 2026-09-02T21:08:03 from an actual suite run: **147/149 tests passed**, **785 observations** recorded.
...
| 5. Improvement & governance | **proven** | 20/20 | 97 |
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13318 token window in 1.6s, cut marked
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 12780m > 1m
...
- `test_runbook.py` **[goal g-20260902-210039]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260902-210039]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260902-210118]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260902-210118]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
**Verdict: proven** — 20 of 20 declared tests ran and passed, producing 97 observations.

<details><summary>What the tests observed (97)</summary>
...
- `test_package.py` **[runnable]** the archive carries 116 modules, 149 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[evidence]** all 149 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1788396977-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 78,804 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 19 capabilities; the README badges carry 149 tests and 32 mutations
...
- `test_invariants.py` **[control-plane]** the seal is derived from fileauth's zone model: all 32 control shapes sealed, the workspace untouched, every path with a declared treatment
...
- `test_invariants.py` **[sandboxes]** 184 sandbox names across 149 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
...
- `test_invariants.py` **[prose]** ARCHITECTURE/README/REFERENCE counts verified against the tree itself: 112 modules, 149 acceptance tests
...
- `test_promotion_leakage.py` **[static]** all 31 trust-defining paths classify CONTROL or RUNTIME — none is workspace
- `test_promotion_leakage.py` **[dynamic]** the file authority refused an agent write to every one of the 31 paths and still admits the harness
...
- `test_controlplane.py` **[matrix]** 106 shell commands from a role holding run_command — truncate, append, delete, create and a redirect — against all 32 control paths fileauth declares: not one durable change, every attempt reported exit=3, 106 tamper events on the record
...
- `test_controlplane.py` **[docker]** 31 read-only bind(s) layered over /work cover every one of the 32 control paths; on that backend the boundary is the kernel's, not a check's
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname 'fe7482ba610f' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94629 -> 198709 bytes (2.1x)
- `test_endurance.py` **[latency]** per-task wall time across 6 batches: 0.20s, 0.19s, 0.22s, 0.22s, 0.19s, 0.19s — median 0.20s, and the last batch is not an outlier: the loop does not get slower as its own history grows
