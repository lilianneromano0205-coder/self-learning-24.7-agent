...
Generated 2026-09-03T02:26:22 from an actual suite run: **151/153 tests passed**, **813 observations** recorded.
...
- `test_chaos.py` **[size]** an 11 MB file, 1,000 atoms and 200 skills compiled to a 13325 token window in 1.2s, cut marked
...
- `test_contract.py` **[goal g-clock]** BLOCKED on budget: wall-clock 13099m > 1m
...
- `test_runbook.py` **[goal g-20260903-021911]** pursuit on free-rider
- `test_runbook.py` **[goal g-20260903-021911]** VERIFIED by runbook weekly-artifact — zero model calls
...
- `test_swarm.py` **[goal g-20260903-021951]** pursuit on parallel-rider
- `test_swarm.py` **[goal g-20260903-021951]** VERIFIED by runbook left-maker, right-maker — zero model calls
...
- `test_package.py` **[runnable]** the archive carries 119 modules, 153 tests, the prompts and settings.toml — and unzipped into an empty directory it passes `harness.py --check` with no setup at all
...
- `test_package.py` **[evidence]** all 153 registered tests are classified into 13 systems with no overlap and no drift, every system states a blind spot, and the standing 'every call is a mock' caveat is in the module and in the generated report
...
- `test_mcp.py` **[sees]** an image block is written to tmp/ (mcp-1788416123-1.png) and the result names the exact `ingest.py vision` command that reads it, so a screenshot becomes something the agent can answer questions about; an undecodable blob is reported as gone, not hidden
...
- `test_backup.py` **[compounding]** four snapshots into the DEFAULT output directory stayed flat at 78,812 bytes with zero nested archives — a backup no longer archives its own backups, which on a 24/7 fleet filled the disk the fleet needs in order to save itself
...
- `test_ledger_defects.py` **[prose]** REFERENCE names 24 templates and all 7 intention kinds; MANUAL and REFERENCE say 19 capabilities; the README badges carry 153 tests and 32 mutations
...
- `test_invariants.py` **[sandboxes]** 188 sandbox names across 153 test files, every one claimed by exactly one file — a shared temp directory is the failure that only shows up under load
...
- `test_invariants.py` **[prose]** ARCHITECTURE/README/REFERENCE counts verified against the tree itself: 115 modules, 153 acceptance tests
...
- `test_docker_live.py` **[isolated]** the command ran inside a Debian container on python 3.12.14, under its own hostname 'c334e859e8ab' which is not this machine's, on a Windows host running python 3.14 — this is not the host backend wearing a different name
...
- `test_endurance.py` **[queue]** 120 tasks completed; the hot queue held 20 then 42 against a retention of 20, 78 moved to the append-only archive with none lost, and state.json went 94595 -> 198696 bytes (2.1x)
